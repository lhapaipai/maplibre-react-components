...
The `mapStyle` is a reactive value. You can specify the url of a style file or directly an object. If we propose an object
...
You can use all options from the MapLibre `MapOptions` type as props for the `RMap` component. Some of these options are responsive and some are not. To distinguish them, non-reactive options have been prefixed with `initial` and event handlers are prefixed with `on`.
...
You can free yourself from this rule for callbacks (type `MapCallbacks`). They were implemented following the React principle of [useEffectEvents](https://react.dev/reference/react/experimental_useEffectEvent).
</Flash>
...
`MapProps` type come from the MapLibre `MapOptions` type. They have been renamed following the conventions described below.

Check the [official documentation](https://maplibre.org/maplibre-gl-js/docs/API/type-aliases/MapOptions/) to get more details.
