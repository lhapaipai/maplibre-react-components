...
import { extend, warnOnce } from "maplibre-gl/src/util/util";
import { checkGeolocationSupport } from "maplibre-gl/src/util/geolocation_support";
import { LngLat } from "maplibre-gl/src/geo/lng_lat";
import { Marker } from "maplibre-gl/src/ui/marker";

import type { Map } from "maplibre-gl/src/ui/map";
import type { FitBoundsOptions } from "maplibre-gl/src/ui/camera";
import type { IControl } from "maplibre-gl";
import { LngLatBounds } from "maplibre-gl/src/geo/lng_lat_bounds";
...
export class GeolocateControl extends Evented implements IControl {
  ...
  constructor(options: GeolocateControlOptions) {
    console.log("GeolocateControl constructor");
    super();
  ...
  onAdd(map: Map) {
    ...
    checkGeolocationSupport().then((supported) => {
      console.log("GeolocateControl setupUI");
  ...
  _setupUI = (supported: boolean) => {
    ...
    this._container.addEventListener("contextmenu", (e: MouseEvent) => {
      console.log("contextmenu prevent default");
      e.preventDefault();
    });
